...
5. **If priorities were added tomorrow (high-priority jobs jump the queue), which parts of your design survive unchanged and which break?**
   ...
   **Breaks or needs real work:**
   ...
   - `list`/`status`/`dlq list` output (and the JSON shape jobs are marshaled into) would want to surface priority to be useful — not a breaking change, but real, visible surface area that has to be extended (new flag/column, new `Job` field, new formatting in `HandleList`).
